...
import { getKompendiumOdzywianiaSchemas } from "../seo/schemas";
import {
  KOMPENDIUM_ODZYWANIA_PATH,
  kompendiumSeo,
  kompendiumLead,
  kompendiumArticleHeadline,
} from "../data/kompendiumOdzywianiaMeta";
import { kompendiumSections } from "../data/kompendiumSections";

export default function KompendiumOdzywianiaPage() {
  const schemas = getKompendiumOdzywianiaSchemas({
    headline: kompendiumArticleHeadline,
    description: kompendiumSeo.description,
  });

  return (
    <div className="font-sans antialiased min-h-screen flex flex-col bg-[#FAFAF5]">
      <Seo
        title={kompendiumSeo.title}
        description={kompendiumSeo.description}
        keywords={kompendiumSeo.keywords}
        url={KOMPENDIUM_ODZYWANIA_PATH}
        type="article"
      />
      <Helmet>
        ...
        <meta property="article:section" content="Medycyna chińska — żywienie i styl życia" />
      </Helmet>
      ...
      <main className="flex-1">
        <header className="pt-24 md:pt-28 pb-10 md:pb-14 bg-gradient-to-b from-[#F5F5DC]/80 to-[#FAFAF5] border-b border-[#71797E]/10">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-10">
            ...
            <p className="mt-3 text-sm text-[#71797E]">
              Aktualizacja: marzec 2026 · Treść edukacyjna — nie zastępuje indywidualnej konsultacji TCM.
            </p>
          </div>
        </header>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-10 md:py-14 flex flex-col lg:flex-row gap-10 lg:gap-14">
          <nav
            aria-label="Spis treści kompendium żywienia TCM"
          ...
          <article className="flex-1 min-w-0 max-w-3xl">
            {kompendiumSections.map((section) => (
              <section
                key={section.id}
                id={section.id}
